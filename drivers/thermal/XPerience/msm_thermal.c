 ...
 *
 * Added code to make more intelligent throttlig
 * Copyright (C) 2016-2017 The XPerience Project (TeamMEX-XDA-Developers)
 * Alessa thermal a thermal driver for 3.10 kernel version 1.0
 */
...
static int __ref set_enabled(const char *val, const struct kernel_param *kp)
{
	...
	if (*val == '0' || *val == 'n' || *val == 'N') {
		...
		pr_info("%s: Alessa Thermal disabled!\n", KBUILD_MODNAME);
	} else {
		if (!enabled) {
			enabled = 1;
			schedule_delayed_work(&check_temp_work,
...
MODULE_DESCRIPTION("AlessaThermal a custom thermal driver for Qualcomm based SOCs");
